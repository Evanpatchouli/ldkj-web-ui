...
`trigger` 支持 `"click"`、`"hover"` 或两者组合的数组。`click` 模式下支持点外部关闭和 `Esc` 关闭，`hover` 模式下进入按钮组时展开、移出时收起。展开方向上的间距由菜单透明命中区域承接，鼠标可以直接移入选项。
...
- `hover` 模式的菜单间距同时作为透明命中区域；移入该区域或菜单项不会提前关闭菜单，点击菜单项后才进入退出动画并卸载。
- 菜单关闭时会先播放子项退出动画，动画完成后才从 DOM 移除；重新移入菜单区域会取消关闭并恢复展开流程。
